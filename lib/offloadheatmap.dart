...
class OffloadHeatmapWidget extends StatefulWidget {
  const OffloadHeatmapWidget({super.key});
  @override
  offloadHeatmapWidgetState createState() => offloadHeatmapWidgetState();
}
class offloadHeatmapWidgetState extends State<OffloadHeatmapWidget> {
  TextEditingController textEditingController =
      TextEditingController(text: "BACK");
      final Set<int> selectedCells = {}; // Set to track selected cells
  final GlobalKey _paintKey = GlobalKey(); // Key for the CustomPaint widget

  // Tap handler to update selected cells
  void handleCellTap(int cellNumber) {
    setState(() {
      if (selectedCells.contains(cellNumber)) {
        selectedCells.remove(cellNumber);
      } else {
    ...
    children: [
      Expanded(
        child: Stack(
          children: [
            Transform.rotate(
              angle: -pi / 2,  // Rotate the entire heatmap by -90 degrees
              child: CustomPaint(
                key: _paintKey,
                painter: HeatMapPainter(
                  cellVertices: scaledCellVertices,
            ...
            GestureDetector(
              ...
              onTapDown: (details) {
                ...
                // Check if any text bounding box contains the tapped position
                HeatMapPainter painter = HeatMapPainter(
                  cellVertices: scaledCellVertices,
                  centers: scaledCenters,
                  cellNumbers: cellNumbers,
                  cellPressures: cellPressures,
                ...
                int? tappedCell = painter.getTappedCell(localPosition, renderBox.size);
                if (tappedCell != null) {
      ...
      Container(
        ...
        decoration: BoxDecoration(
          color: const Color.fromARGB(255, 63, 89, 234),
          borderRadius: BorderRadius.circular(15),
          border: Border.all(color: const Color.fromARGB(255, 63, 89, 234)),
        ),
        child: DropdownButtonHideUnderline(
          child: DropdownButton<String>(
            ...
            dropdownColor: const Color.fromARGB(255, 63, 89, 234),
            ...
            items: ['1.0', '1.5', '2.0'].map((String value) {
              return DropdownMenuItem<String>(
                value: value,
                child: Text('Scale $value', style: const TextStyle(fontSize: 24)),
              );
            }).toList(),
          ),
        ),
...
class HeatMapPainter extends CustomPainter {
  ...
  HeatMapPainter({
    required this.cellVertices,
    required this.centers,
    required this.cellNumbers,
    required this.cellPressures,
  ...
  //     final path = Path();
  //     List<Offset> transformedVertices = cellVertices[i];

  //     path.addPolygon(transformedVertices, true);

  //     final cellNumber = cellNumbers[i];
  //     final pressureValue = cellPressures[cellNumber] ?? 0;
  //     final convertedPressure = pressureValue * 6.89;

  //     // paint.color = selectedCells.contains(cellNumber)
  //     //     ? Colors.green // Highlight tapped cells in green
  //     //     : getColorFromPressure(convertedPressure);
  //     paint.color = getColorFromPressure(convertedPressure); // Set cell color based on pressure only
  //     canvas.drawPath(path, paint); // Draw cell with its pressure-based color
  ...
  //         color: Colors.white,
  //         fontSize: 14,
  //         fontWeight: selectedCells.contains(cellNumber) ? FontWeight.bold : FontWeight.normal,
  //       ),
  //     );
  ...
  //     final Offset textCenter = calculateCentroid(transformedVertices);
  //     final Offset textStart = Offset(
  //       textCenter.dx - (textPainter.width / 2),
  //       textCenter.dy - (textPainter.height / 2),
  //     );
  //     textPainter.paint(canvas, textStart);
  //       // Draw tick mark if cell is selected
  //     if (selectedCells.contains(cellNumber)) {
  ...
  //       final tickPath = Path();
  //       tickPath.moveTo(textCenter.dx - 5, textCenter.dy);
  //       tickPath.lineTo(textCenter.dx, textCenter.dy + 5);
  //       tickPath.lineTo(textCenter.dx + 10, textCenter.dy - 5);
...
void paint(Canvas canvas, Size size) {
  ...
  for (int i = 0; i < cellVertices.length; i++) {
    ...
    // Set cell color based on pressure only
    paint.color = getColorFromPressure(convertedPressure);
    canvas.drawPath(path, paint);

    final Offset textCenter = calculateCentroid(transformedVertices);

    // Draw tick mark if cell is selected, otherwise draw text
    if (selectedCells.contains(cellNumber)) {
      final tickPaint = Paint()
        ..color = const Color.fromARGB(255, 255, 255, 255)
        ..strokeWidth = 2.0
      ...
      // Draw text for non-selected cells
      final textSpan = TextSpan(
        text: cellNumber.toString(),
  ...
  // New method to check if a tap falls within any cell polygon
  int? getTappedCell(Offset tapPosition, Size canvasSize) {
    for (int i = 0; i < cellVertices.length; i++) {
      final path = Path();
      path.addPolygon(cellVertices[i], true);
